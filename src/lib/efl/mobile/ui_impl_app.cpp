...
{
	ui_impl_app *app = static_cast<ui_impl_app *>(data);
...
void ui_impl_app::on_lang_changed(app_event_info_h event_info)
{
	this->app->on_lang_changed(event_info);
}

void ui_impl_app::on_low_memory(app_event_info_h event_info)
{
	this->app->on_low_memory(event_info);
}

void ui_impl_app::on_low_battery(app_event_info_h event_info)
{
	this->app->on_low_battery(event_info);
}

void ui_impl_app::on_region_changed(app_event_info_h event_info)
{
	this->app->on_region_changed(event_info);
}

void ui_impl_app::on_orient_changed(app_event_info_h event_info)
{
	this->app->on_orient_changed(event_info);
}

bool ui_impl_app::on_create()
{
	return this->app->on_create();
}

void ui_impl_app::on_pause()
{
	this->app->on_pause();
}

void ui_impl_app::on_resume()
{
	this->app->on_resume();
}

void ui_impl_app::on_control(app_control_h app_control)
{
	this->app->on_control(app_control);
}

void ui_impl_app::on_terminate()
{
	this->app->on_terminate();
}

ui_impl_app::~ui_impl_app()
{
	delete (this->viewmgr);
...
ui_impl_app::ui_impl_app(ui_app *app, const char *pkg, const char *locale_dir)
		: app(app)
{
...
ui_viewmgr *ui_impl_app::get_viewmgr()
{
	return this->viewmgr;
}

bool ui_impl_app::run(int argc, char **argv)
{
...
bool ui_impl_app::activate()
{
	return this->viewmgr->activate();
}

bool ui_impl_app::deactivate()
{
	return this->viewmgr->deactivate();
}
...
{
	this->impl = new ui_impl_app(this, pkg, locale_dir);
}
...
{
	return ui_impl_app::get_instance()->get_viewmgr();
}
