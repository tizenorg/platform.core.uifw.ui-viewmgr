...
Evas_Object *ui_viewmgr::set_transition_layout(string transition_style)
{
	...
	elm_object_part_content_unset(this->layout, "pcontent");
	elm_object_part_content_unset(this->layout, "content");
	...
	if (!effect_layout)
	{
		...
		elm_object_part_content_unset(this->conform, "elm.swallow.content");
		this->create_base_layout(this->conform, transition_style.c_str());

		effect_map.insert(pair<string, Evas_Object *>("zoom", this->layout));
	}
	else
	{
