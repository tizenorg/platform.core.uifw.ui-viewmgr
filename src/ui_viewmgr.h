...
namespace efl
{
   class ui_viewmgr : public ui_viewmgr_base<Evas_Object *>
   {
    ...
    public:
       ui_viewmgr(Evas_Object *window) : animation_job(NULL), animation_ongoing(false), to_show(false)
       {
          if (!window) {
       ...
       virtual ui_view_base<Evas_Object *>* pop_view()
       {
          LOGI("CALLED");
          ui_viewmgr_base :: pop_view();

		  this->to_show = false;
		  ui_view_base<Evas_Object *> *view = ui_viewmgr_base :: pop_view();
          if (!this->animation_job)
            this->animation_job = ecore_job_add(_animation_cb, this);
