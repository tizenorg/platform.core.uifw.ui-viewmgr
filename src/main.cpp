...
/*
	Evas_Object *content = elm_layout_add(ad->win);
	elm_layout_theme_set(content, "layout", "nocontents", "default");
	evas_object_size_hint_weight_set(content, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
	evas_object_size_hint_align_set(content, EVAS_HINT_FILL, EVAS_HINT_FILL);
	elm_object_part_text_set(content, "elm.text", "View 1");
	elm_layout_signal_emit(content, "align.center", "elm");
	evas_object_show(content);
*/
	...
	evas_object_event_callback_add(content, EVAS_CALLBACK_MOVE, _resize, NULL);
	evas_object_event_callback_add(content, EVAS_CALLBACK_RESIZE, _resize, NULL);
